...
def test_effective_rank_collinear_is_one():
    ...
    cm = np.ones((4, 4))
    assert effective_rank(cm) == pytest.approx(1.0)
...
def test_effective_rank_from_returns_independent():
    ...
    mat = rng.standard_normal((500, 4))
    er = effective_rank_from_returns(mat)
...
def test_effective_rank_from_returns_duplicated_column():
    ...
    mat = np.hstack([base, base.copy(), indep])  # col0 == col1
    er = effective_rank_from_returns(mat)
...
def test_resume_gate_column_alignment_validation():
    mat = np.zeros((10, 2))
    with pytest.raises(ValueError):
        resume_gate([0.01, 0.02, 0.03], mat)  # 3 p-values, 2 columns
