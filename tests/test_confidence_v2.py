...
def test_confidence_components_in_unit_interval() -> None:
    c = compute_confidence_v2(
        ...
        is_sharpe=0.629,
        oos_sharpe=0.629,
        days_since_last_calibration=10.0,
        rolling_aucs=[0.58, 0.59, 0.59, 0.6],
    )
    for v in (
        c.sample_quality,
        c.effective_size_factor,
        c.score_stability,
        c.return_consistency,
        c.recency_quality,
...
def test_composite_is_geometric_mean_of_components() -> None:
    ...
    expected = math.exp(
        (
            math.log(c.sample_quality)
            + math.log(c.effective_size_factor)
            + math.log(c.score_stability)
            + math.log(c.return_consistency)
            + math.log(c.recency_quality)
        )
        / 5.0
    )
