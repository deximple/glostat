...
def test_high_n_stable_thesis_keeps_higher_weight() -> None:
    ...
    p_low = predict(
        ticker="X",
        horizon="swing_30d",
        contributions=(sig_low,),
        cal_table=table_low,
    )
    p_high = predict(
        ticker="X",
        horizon="swing_30d",
        contributions=(sig_high,),
        cal_table=table_high,
    )
...
def test_stale_calibration_dampens_weight() -> None:
    ...
    p_fresh = predict(
        ...
        horizon="swing_30d",
        contributions=(sig,),
        cal_table=fresh_table,
    )
    p_stale = predict(
        ticker="X",
        horizon="swing_30d",
        contributions=(sig,),
        cal_table=stale_table,
    )
...
def test_oos_degraded_thesis_dampens_weight() -> None:
    ...
    p_clean = predict(
        ...
        horizon="swing_30d",
        contributions=(sig,),
        cal_table=clean_table,
    )
    p_degraded = predict(
        ticker="X",
        horizon="swing_30d",
        contributions=(sig,),
        cal_table=degraded_table,
    )
...
def test_confidence_v2_components_reflect_thesis_quality() -> None:
    ...
    p_strong = predict(
        ticker="X",
        horizon="swing_30d",
        contributions=(sig_strong,),
        cal_table=table_strong,
    )
...
def test_sk_innovation_mock_high_n_thesis_gets_higher_weight() -> None:
    ...
    p_high = predict(
        ticker="096770",
        horizon="swing_30d",
        contributions=(high_n_sig,),
        cal_table=high_n_table,
    )
    p_low = predict(
        ticker="096770",
        horizon="swing_30d",
        contributions=(low_n_sig,),
        cal_table=low_n_table,
    )
