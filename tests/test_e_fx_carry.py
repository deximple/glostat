...
def _patch_cache_with(cache: PriceCache, mapping: dict[str, list[tuple[date, float]]]) -> None:
...
def _empty_cache(tmp_path: Path) -> PriceCache:
    ...
    return PriceCache(
        client=yf, start=date(2024, 1, 1), end=date(2024, 12, 31), cache_dir=tmp_path / "ohlcv"
    )
