...
| **INV-GS-133**  | **Composite weighting: `_weight_for()` multiplies brier_weight by OOS-stability factor `max(0.10, 1 - 0.9·clip(oos_deg,0,1))`. Theses with OOS_deg ≥ 100% get weight floored to 10% of brier. Closes the audit finding that 5 of 8 measured theses had IS-only edge that died OOS but carried full Brier weight. E_PEAD 0.172→0.017, KEPT: E_FOREIGN_REVERSAL/E_FUNDAMENTAL/E_TIME.** | **active v1.10.4**                                                |
| **INV-GS-134**  | **E_VKOSPI_MOOD_KR: Lee/Son/Lee 2024 + KRX 2009 — KR asymmetric event-conditional mood (VKOSPI↑+r<-10% → reversal LONG; VKOSPI↓+r>+10% → drift LONG; misaligned/below-threshold → NEUTRAL). Universe: KOSPI 200. Small-cap 1.5× multiplier. 12 caveats including alpha-decay risk and calibration-not-prediction framing. n=0 bootstrap; live VKOSPI fetcher deferred via pluggable provider (KRX session-cookie/Naver scrape).** | **active v1.10.6**                                                |
| **INV-GS-135**  | **phase_kr_vkospi_mood hindcast harness + YFinanceReturnResolver + KospiSmallCapResolver + `glostat kr-vkospi-hindcast` CLI. Basket-mode (1 trade/day) prevents shared ΔVKOSPI autocorrelation. Requires VKOSPI CSV (docs/VKOSPI_SETUP.md). Calibration loader picks up `cache/hindcast/phase_kr_vkospi_mood/e_vkospi_mood_kr_report.json` to lift E_VKOSPI_MOOD_KR from n=0 bootstrap to measured.** | **active v1.10.8**                                                |
